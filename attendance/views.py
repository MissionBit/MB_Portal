...
def attendance(request):
    if request.method == "POST":
        if request.POST.get("attendance_taken") is not None:
            ...
            context = take_attendance_context(request.POST.get("course_id"),
    ...
    if request.GET.get("course_id") is not None:
        ...
        context.update({"attendance_statistic": get_course_attendance_statistic(request.GET.get("course_id"))})
    else:
        attendance_averages = compile_attendance_averages_for_all_courses()
        context = {"classrooms": Classroom.objects.all(),
                   "attendance_averages": attendance_averages}
...
def get_classroom_attendance(course_id):
    ...
    return {"classroom_attendance": classroom_attendance,
            "dates": dates,
            "course": course,
            "course_id": course_id,
...
def get_course_attendance_statistic(course_id):
    ...
    class_attendance = [daily_attendance for daily_attendance in class_attendance if daily_attendance.date < datetime.today().date()]
...
def get_average_attendance_from_list(list):
    return sum(attendance_object.presence == "Present" or attendance_object.presence == "Late"
               for attendance_object in list) / len(list)


def get_classroom_meeting_dates(course_id):
    return class_offering_meeting_dates(ClassOffering.objects.get(name=Classroom.objects.get(id=course_id).course))
